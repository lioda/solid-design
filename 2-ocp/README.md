...
- new stats: compose seller stats with another thing to create a new report with objectives
- subset of features (inheritance or composition)
...
We could modify the `ManagementReporter` class to let it know in which context it runs, doing something like this:

```
createReport(context: 'HumanResource' | 'SalesManagement', prices: Record<Product, number>, accountingDate: AccountingDate): string {
  // ... generate common report
  if (context === 'HumanResource') {
    // ... generate HR specific report
  }
}
```

- SRP violation
- using only one value of `context`

=> create a subclass (more complete as super class) or compose several components (HR / management are 2 behaviors that we can compose in an ultimate class)

~~But this solution is an infringement of SRP: we can't let a~~
